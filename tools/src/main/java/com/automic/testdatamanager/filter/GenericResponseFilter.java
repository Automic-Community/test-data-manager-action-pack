...
public class GenericResponseFilter extends ClientFilter {
	...
	private static final String RESPONSE_MSG = RESPONSE_CODE
			+ " Message : [%s]";
	...
	public ClientResponse handle(ClientRequest request) {
		...
		if (response.getClientResponseStatus() != null
				&& CommonUtil.checkNotEmpty(response.getClientResponseStatus()
						.getReasonPhrase())) {
			msg = String.format(RESPONSE_MSG, response.getStatus(), response
